...
import {type AddressRequest, type ParkingRequest, ParkingType} from "@/lib/typedef";
...
const CreateParkingForm = () => {
    ...
    const handleSubmit = async (data: z.infer<typeof FormSchema>) => {
        const parkingBody: ParkingRequest = {
            capacity: Number(data.capacity),
            freePlaces: Number(data.freePlaces),
            type: data.type === "PUBLIC" ? ParkingType.PUBLIC : ParkingType.PRIVATE,
        ...
        const addressBody: AddressRequest = {
            street: data.street,
            city: data.city,
            longitude: myLongitude,
            latitude: myLatitude,
            parkingId: parkingResult.id,
        };

        const addressResponse = await fetch(
            `${BASE_URL}/addresses?parking_id=${parkingResult.prkId}`,
            {
                method: "POST",
                headers: {"Content-Type": "application/json"},
